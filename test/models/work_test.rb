...
describe Work do
  ...
  describe "vote_count" do
    ...
    it "tracks the number of votes" do
      work = Work.create!(title: "test title", category: "movie", user: users(:dan))
      4.times do |i|
        user = User.create!(username: "u__#{i}", provider:"github",uid:"100#{i}")
  ...
  describe "top_ten" do
    before do
      ...
      test_users = []
      20.times do |i|
        test_users << User.create!(username: "u_#{i}", provider:"github",uid:"1200#{i}")
      end

      # Create media to vote upon
      Work.where(category: "movie").destroy_all
      8.times do |i|
        work = Work.create!(category: "movie", title: "test movie #{i}", user: users(:dan))
